...
function Lexer(source) {
  ...
  let _skipWhitespace = function _skipWhitespace(charsArg) {
    let chars = charsArg;
    let c1 = chars[0];
  ...
  let _skipComments = function _skipWhitespace(charsArg) {
    let chars = charsArg;
    let c1 = chars[0];
    let c2 = chars[1];

    let commentEntry = Lexicon.comments.filter(x => x[0] == c1 || x[0] == c2);
    while (commentEntry.length > 0) {
      ...
      commentEntry = Lexicon.comments.filter(x => x[0] == c1 || x[0] == c2);
    }
    return chars;
  };
  ...
  let _skipWhitespaceAndComments = function _skipWhitespaceAndComments(charsArg) {
    ...
    do {
      chars = newChars;
      newChars = _skipWhitespace(newChars);
